  ...
  private class CommandHandler implements CliParser.CommandHandler {
    ...
    @Override public void load (String filename) {
      ...
      try {
        ...
        if (startPC!=null)
          ...
          System.out.printf ("File %s has no instructions\n", filename);
      } catch (FileNotFoundException e) {
        System.out.printf ("File %s not found.\n", filename);
      } catch (Memory.FileTypeException fte) {
        System.out.printf ("File %s must end in '.s' for assembly or '.machine' for machine code\n", filename);
      } catch (Memory.InputFileSyntaxException ifse) {
        System.out.printf ("%s error in file %s\n", ifse.toString (), filename);
      } catch (Exception ex) {
    ...
    @Override public void assertMem (int addr, int value, String desc) {
      ...
      if (machine.memory.regionForAddress (addr) != null) {
        try {
          int memValue = machine.mainMemory.readInteger (addr);
          if (memValue != value) {
            System.out.printf ("XXX ASSERTION FAILURE%s: *(0x%x) == 0x%x != 0x%x\n", desc!=null? " ("+desc+")" : "", addr, memValue, value);
          }
        } catch (AbstractMainMemory.InvalidAddressException e) {
          System.out.printf ("XXX ASSERTION FAILURE: Address out of bounds.\n");
          throw new CommandException ();
        }
      } else {
        System.out.printf ("XXX ASSERTION FAILURE: Address out of bounds.\n");
    ...
    @Override public void setIns (int addr, CliParser.CommandHandler.InsOper oper, String value) {
      ...
      if (region==null) {
        if (addr < 0 || addr + 3 > machine.mainMemory.length() || addr % 2 != 0) {
          ...
          region = machine.memory.regionForAddress (addr);
          assert (region != null);
        }
      ...
      if (row == -1) {
        System.out.printf ("Address illegally overlaps with existing instruction.\n");
        throw new CommandException ();
      }
      try {
        switch (oper) {
    ...
    @Override public void help () {
      ...
      System.out.print ("  a|assert \"description\"\n");
      System.out.print ("  a|assert <mem-address> == value \"description\"\n");
      System.out.print ("  a|assert <register> == value    \"description\"\n");
